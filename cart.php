			...
			if(isset($_POST["checkout"])){
				if(isset($_SESSION["shopping_cart"])){
                    foreach($_SESSION["shopping_cart"] as $keys => $values) {
                        ...
                        $usernamef=$_SESSION['username'] ;$orderid=uniqid();$_SESSION['orderid']=uniqid();
                        $sql = "INSERT INTO `order`(`orderid`, `category`, `quantity`, `price`, `Buyer`, `productid`, `status`) VALUES 
							('$orderid','$category','$quantity','$price','$usernamef','$id', 'PENDING')";
                        if ($conn->query($sql) === TRUE) {

                        }else{ echo "Error Adding data to the table order: " . $conn->error;}
                    }
...
<head>
	...
	<div class="row text-center">
        ...
        $c=mysqli_fetch_assoc($result)["total"];
			$rand = rand(6, $c) - 6;
