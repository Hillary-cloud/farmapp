...
if (isset($_POST["AddProduct"])) {
    if (!empty($_POST['phone']) && (!empty($_POST['select_category'])) && (!empty($_POST['category'])) && (!empty($_POST['price']))) {
        ...
        $sql = "UPDATE products SET CompanyName = '$_POST[username]', 
					Category='$_POST[category]', 
					Description='$_POST[description]', 
					Phone='$_POST[phone]', image='$file', Price='$_POST[price];'
				    WHERE id = '$id'";
...
if (isset($_POST["imageUpdate"])&&isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $sql = "SELECT * FROM products WHERE id = '$id'";
    $result = $conn->query($sql);
...
// output data of each row
    while ($row = $result->fetch_assoc()) {
        if ($_FILES['image']['name'] == 0) {
            ...
            $image = str_replace("'", "''", file_get_contents($image));

            $sql = "UPDATE products SET image = '$file' WHERE id = '$id' ";
            if ($conn->query($sql) === TRUE) {
            }
...
<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
    <div class="container">
        <div class="navbar-header">
            ...
            <a class="navbar-brand" href="#"><strong>Farm Link</strong></a>
        </div>
...
<h3 align="center">You are logged in
    as <?php if (isset($_SESSION['CompanyName'])) echo $_SESSION['CompanyName']; ?></h3>
</br>
</br>
<?php
if ($result->num_rows > 0) {
...
<form method="post" enctype="multipart/form-data" action="" style="width:50%; margin: auto;">
    ...
    <div class="form-group ">
        ...
        <form method="post" enctype="multipart/form-data" style="width:50%; margin: auto;">
            ...
            <p></p><?php echo '<img  style = " border-radius: 10px;height: 45px; width: 45px;"src =' . $row["image"] . '">'; ?>
