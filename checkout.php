...
//				}
	...
	if(isset($_POST["orderProduct"])){
            ...
            echo "Error Adding data to the table delivering--------------------------------------------------------
            ------------------------------------------------------------------------------------------------------
            ------------------------------------------------------------------------------------------------------
            ------------------------------------------------------------------------------------------------------
            ------------------------------------------------------------------------------------------------------
...
<div class = "container">
		...
		$sql =	"SELECT order.category, order.quantity, order.price, products.image, products.CompanyName FROM `order`, `products` WHERE 
			order.Buyer = '$usernamef' AND order.status =  'PENDING' AND products.id = order.productid ";
		...
		?>
				...
				<tr>
					<th width = "10%"><?php echo '<img  style = " border-radius: 10px;height: 45px; width: 45px;"src = "'.$row["image"].'">'; ?></th>
